...
### 25. Isolation + integration suites are latently red on `main` — and CI does not run them

- **Severity:** Medium — one item is security-relevant (cross-tenant guard); the rest are local test hygiene. No runtime/data impact. Discovered during CP-2b (verified by stashing CP-2b and re-running: all reproduce on clean `main`, so **none are caused by CP-2b**).
- **Owner:** Founder to prioritise; small fix + CI-wiring proposed.
