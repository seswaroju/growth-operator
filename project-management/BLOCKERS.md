...
### 22. Tier engine reads `approval_policies` by action only (no pack/org scope) — latent multi-pack + test-isolation coupling

- **Severity:** Low for the MVP — **correct for the single-pack MVP** (every org uses the one shared jewelry pack, so its `scope='pack'` policies *should* apply to all of them). Becomes a real bug only when a **second active pack** coexists.
- **Owner:** Engineering (a future ticket, once a second production pack is in scope) — needs founder approval since it changes tier resolution.
- **Description:** `core/approvals/engine.py::_contributors` selects `FROM approval_policies WHERE action_type = :at` with **no `pack_id` / `org_id` filter**. With one shared pack this is intentional; with two active packs, pack A's send policy would govern pack B's runs (cross-pack tier leak). It also couples test isolation: any test that leaves `scope='pack'` policies behind changes every *other* test's tier evaluation. Surfaced by the A1 E2E journey (`test_jewelry_journey.py`): its installed jewelry `action.message.send` tier-2 `cel='true'` policy leaked into `test_send_loop::test_full_run_auto_sends_through_real_proxy`, forcing a plain reply (should auto-send, tier 1) to park (tier 2). **Confirmed pre-existing** — reproduced with all A1 core changes stashed.
- **Mitigation (done in A1):** the E2E journey now deletes its pack's `approval_policies` at teardown (`DELETE … WHERE pack_id=…`; `install()` re-seeds idempotently), so it no longer pollutes sibling tests.
