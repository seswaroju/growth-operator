...
### 12. WhatsApp media — real clamav/MinIO adapters WIRED; live verify pending image pull
...
- **Remaining (environment):** the `clamav`/`minio` images did not pull on this machine — Docker's macOS credential helper cancelled the Docker Hub auth prompt (`User canceled the operation`). Founder resolves the credential helper (or `docker login`), then `docker compose --profile media up clamav minio` and re-run `pytest tests/integration/test_media_adapters.py` to verify the 4 live tests. Meta media download/upload stays gated (#3).
