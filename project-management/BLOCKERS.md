...
- **Known limitation:** the vault-drift checks are now **local-only** (they can't run in CI without the private vault). If CI enforcement is wanted, vendor `tool-permissions.yaml` + `topics.yaml` into the repo (a founder decision — they currently live only in the vault).
