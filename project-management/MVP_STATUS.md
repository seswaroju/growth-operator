...
| MVP-043 | Kirana dry-run CI gate | ✅ | 2026-07-31 | _uncommitted_ | `installer.dry_run` (full pipeline in a rolled-back txn, validates+plans, no writes); `verticals/kirana/install.yaml` + `tests/e2e/test_kirana_dryrun.py`; CI gate proves 2nd pack installs with zero core changes; **291 pytest** |
