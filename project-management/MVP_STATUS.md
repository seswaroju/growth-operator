...
| MVP-065 | Policy engine | ✅ | 2026-08-02 | `(pending)` | Migration `1993ba538f4f` (014: approval_policies custom-RLS + trust_ledger/incident_tightening/execution_token_jti +RLS); `engine.evaluate` (core-tier4→pack→tenant→incident, CEL, **max-tier-wins**, matched ids); **deterministic (10k shuffles)** + CEL compile cache (**p95 0.88ms**); `validate_tenant_rule` **tighten-only**; **mediation tier check now live** (proxy→engine); token minting→066; **433 pytest** |
