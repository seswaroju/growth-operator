...
| MVP-065b | Audit hardening (CEL fail-safe + isolation tests) | ✅ | 2026-08-02 | `(pending)` | `_matches` fails safe on any CEL compile/eval error (rule still counts → can only tighten); + `tests/isolation/test_batch_rls.py` cross-tenant tests for quotes/committed_figures_ledger/approval_policies (probed as app_rw); **436 pytest** |
