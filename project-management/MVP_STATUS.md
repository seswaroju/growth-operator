...
| MVP-042 | Catalog schema registration + index gen | ✅ | 2026-08-01 | _uncommitted_ | Migration `1b9dc38df16c` (`generated_ddl`); `indexes.py` DDL gen from x-index (btree/numeric-cast/GIN) wired into installer + `apply_generated_indexes` CONCURRENTLY (lock_timeout 3s, IF NOT EXISTS); jewelry→6 indexes; **328 pytest** |
