...
| MVP-051 | Rate ingestion + manual entry | ✅ | 2026-08-02 | `(pending)` | `rates.py` `ingest_rate` (bounds → quarantine, staleness clock untouched) + `fetch_and_store` (alert.ops on quarantine / rate.updated on success) + `record_manual_rate` (audited, keys only); gated `SimulatedRateFetcher` default, real IBJA fails closed (#5); `POST /v1/rates/manual` (ORG_MANAGE); staleness→409 via engine gate; no migration; **396 pytest**. Tier-2 approval + scheduler firing deferred |
