...
| MVP-059 | Composer + tenant layer generator | ✅ | 2026-08-01 | _uncommitted_ | `composer.py` base+vertical+tenant render (immutable cache, strict params → MissingParam, sha256 hash reproducible, fail-closed); `tenant_layer.py` settings-baked layer (hash-versioned, idempotent); `prompts/base/concierge.md`; **336 pytest** |
