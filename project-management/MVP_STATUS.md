...
| MVP-048 | Embeddings + hybrid RRF | ✅ | 2026-07-31 | _uncommitted_ | `embed.py` gated-simulated embedder + batch job; `search.py` RRF(k=60) fuse BM25⊕kNN(HNSW `<=>`) + filter pushdown + empty→nearest; `/catalog/search` returns `{results,nearest}`; real provider deferred (#16); **323 pytest** |
