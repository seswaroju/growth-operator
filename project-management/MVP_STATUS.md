...
| MVP-040 | Transactional installer + API | ✅ | 2026-07-31 | _uncommitted_ | `installer.py` 6-step atomic pipeline (2 deferred #14) + digest idempotency + rollback + uninstall; migration `5dcbda42efca` (+`failed` status); `GET/POST/DELETE /v1/packs(/installations)`; **289 pytest** |
