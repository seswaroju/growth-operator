...
| MVP-063 | Failure contract + circuit breaker | ✅ | 2026-08-04 | *pending* | Migration `da3474bd3cdb` (`incidents` +RLS; lands ahead of 018/MVP-074—flagged, DECISIONS; `circuit_open` already an allowed instance status). `failure.py` — `note_failure` (tier-2→incident w/ run link + `trust.record_incident`; 2nd consecutive→**circuit_open** + `alert.ops` + incident), `note_success` reset, `is_circuit_open`, `close_circuit` (reactivate + resolve). Executor: `start_run` **holds** on open circuit; `_drive` retries a hard `provider_unavailable` step **once** then trips; proxy converts a raising tool → `provider_unavailable`. **506 pytest** (+isolation). <30s alert-delivery + retry/threshold config deferred |
