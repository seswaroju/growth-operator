...
| MVP-068 | WhatsApp interactive approvals + ladder | ✅ | 2026-08-03 | `(pending)` | Migration `bb65660f0771` (approvals notification-state cols; ticket said "none"—flagged, DECISIONS); `notify.py` render card + interactive ✅/❌ buttons + consumer on `approval.requested` (gated-sim send); button + text ✅/❌ fallback → `service.resolve`; **ladder** remind 50%/escalate 75%/expire (per-org, registered); **478 pytest**. Real Meta send + scheduler firing + normalizer inbound deferred |
