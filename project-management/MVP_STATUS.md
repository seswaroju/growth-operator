...
| MVP-055 | Executor skeleton + checkpoints | ✅ | 2026-08-02 | `(pending)` | Migration `f124e1102952` (015: agent_runs/steps/memory +RLS, model_routes; runs carry composed_prompt_hash + permission_manifest_hash); **LangGraph** graph route→compose→model_turn→tool_call→respond + durable executor (Redis+PG checkpoint/node, kill/budget/timeout, idempotent respond); gated-sim LLM; `GET /v1/ops/runs/{id}`; **chaos-kill 10/10 no dup send**; langgraph dep approved (DECISIONS); **408 pytest** |
