...
| MVP-039 | Bundle parser + verifier | ✅ | 2026-07-31 | _uncommitted_ | `core/packs/bundle.py`: prompt anchor split → layer records, `parse_pack_dir` (per-file contract validation, path+file-named errors), digest manifest + ed25519 verify (tampered refused), `load_bundle` dev/prod; no new deps (.tar.zst deferred #13); **279 pytest** |
