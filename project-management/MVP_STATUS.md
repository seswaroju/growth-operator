...
| MVP-034 | Send adapter (gated) | ✅ | 2026-07-30 | _uncommitted_ | `send.py`: single outbound exit, 4 fail-closed gates (audit capability / execution token / suppression / consent), 429 Retry-After + 5xx×3 retries, `msg.sent`/`msg.failed` + audit outcome; no migration; **191 pytest** |
| MVP-036 | Suppression + consent | 🟡 | 2026-07-30 | _uncommitted_ | **Enforcement half done in MVP-034** (suppression+consent gate, transactional-exempt). Remaining (in progress): STOP/UNSUB keyword auto-suppress (normalizer, en/hi/te) + transactional confirm reply (auto-send **founder-approved**, DECISIONS 2026-07-30) + suppressed badge (frontend) |
