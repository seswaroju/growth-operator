...
| MVP-037 | Media handling | ✅ | 2026-07-31 | _uncommitted_ | `media.py` inbound mime/size gates + fail-closed AV scan (quarantine+`alert.ops`) + object store → `messages.media`; outbound upload helper; **simulated** scanner/store (no new deps, real clamav/MinIO gated — BLOCKERS #12); normalizer wired; **225 pytest** |
