...
| MVP-041 | Jewelry install e2e fixture | ✅ | 2026-07-31 | _uncommitted_ | `verticals/jewelry/install.yaml` + `tests/e2e/test_jewelry_install.py` (assert expected_result field-by-field, <60s); wired into CI `migrate` job (app_rw + e2e) as a required check; **290 pytest** |
