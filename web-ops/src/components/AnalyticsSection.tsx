...
function wow(now: number, prev: number): { text: string; cls: string } {
  if (prev === 0) return { text: now > 0 ? "new" : "—", cls: "text-muted" };
  const pct = Math.round(((now - prev) / prev) * 100);
  if (pct > 0) return { text: `+${pct}%`, cls: "text-good" };
  if (pct < 0) return { text: `${pct}%`, cls: "text-danger" };
  return { text: "0%", cls: "text-muted" };
}
...
function Metric({ label, value, delta }:
  ...
  return (
    <Card className="p-5">
      <div className="font-serif text-2xl font-medium tnum text-ink">{value}</div>
      <div className="mt-1 text-sm font-medium text-ink-2">{label}</div>
      {delta && <div className={`text-[11px] font-medium ${delta.cls}`}>{delta.text} vs prev period</div>}
    </Card>
  );
}
...
function Executive({ r }: { r: AnalyticsRollup }) {
  return (
    <section>
      <h2 className="mb-3 text-sm font-semibold text-ink">Executive · all stores (last {r.period_days}d)</h2>
...
function Marketing({ r }: { r: AnalyticsRollup }) {
  return (
    <section>
      <h2 className="mb-3 text-sm font-semibold text-ink">Marketing · all stores (last {r.period_days}d)</h2>
  ...
  return (
    <div className="space-y-6">
      {isLoading ? (
        ...
        <p className="text-sm text-danger">Couldn't load analytics — {(error as Error).message}</p>
      ) : data ? (
        <>
          <Executive r={data} />
          <Marketing r={data} />
