...
async def scene() -> AsyncIterator[Scene]:
    ...
    try:
        ...
        # Subscriptions before organizations, and the plan last: this fixture created a plan row
        # per run and never removed it, so 126 had accumulated in the founder's development
        # database. The suite that creates a row is the suite that removes it.
        #
        # By ID, not by name pattern. `LIKE 'CampPlan-%'` would also delete a plan the founder
        # happened to name that way, and a test suite must never be a garbage collector for
